...
public class VerIngredientes extends JFrame{
    ...
    public VerIngredientes(){
        ...
        JLabel Titulo = new JLabel("Ingrediente");
        this.add(Titulo);
        this.setLayout(null);
                    ...
                    if (main.productosA.get(i).getId()== Integer.parseInt(Menu.idP2)){
                        for (int j = 0; j < main.productosA.get(i).getIngredients().size(); j++) {
                            if(main.productosA.get(i).getIngredients().get(j).getName().equals(Menu.nameI)){
                                ...
                                textologA="\n"+dtf.format(LocalDateTime.now())+"\t"+main.username+": Ha visto los ingredientes del producto con id: "+Menu.idP2+".";
